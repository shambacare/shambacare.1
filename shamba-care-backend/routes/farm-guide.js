...
const { Farm, Task, Crop, CropActivity } = require('../models');
...
function getTaskType(stageName) {
    ...
    if (stageName.includes('Spray') || stageName.includes('fungicide')) return 'spraying';
    if (stageName.includes('Fertilizer')) return 'fertilizing';
    if (stageName.includes('Water') || stageName.includes('Irrigation')) return 'irrigation';
    if (stageName.includes('Monitor')) return 'monitoring';
    return 'other';
}

// Get crop growth stages (unchanged)
router.get('/crop-stages/:cropName', verifyToken, async (req, res) => {
    try {
        const cropName = req.params.cropName;
        const growthData = cropGrowthStages[cropName] || cropGrowthStages['Maize'];
...
// ==================== GENERATE TASKS FOR A CROP ====================
router.post('/generate-tasks', verifyToken, async (req, res) => {
    try {
        const { crop_id, farm_id, crop_name, planting_date } = req.body;
        
        // Validate required fields
        if (!crop_id || !farm_id || !crop_name || !planting_date) {
            return res.status(400).json({ success: false, message: 'crop_id, farm_id, crop_name, and planting_date are required' });
        }

        const growthData = cropGrowthStages[crop_name] || cropGrowthStages['Maize'];
        const planting = new Date(planting_date);
        const tasks = [];
        
        for (let i = 0; i < growthData.stages.length; i++) {
            ...
            tasks.push({
                farm_id,
                crop_id,          // ✅ Added crop_id
                task_name: stage.name,
                description: stage.activities.join('. '),
                task_type: getTaskType(stage.name),
                scheduled_date,
                position_order: i,
        ...
        // Delete existing tasks for this crop (to avoid duplicates)
        await Task.destroy({ where: { crop_id } });
...
router.get('/tasks/:cropId', verifyToken, async (req, res) => {
    try {
        const cropId = req.params.cropId;
        const tasks = await Task.findAll({
            where: { crop_id: cropId },
            order: [['position_order', 'ASC'], ['scheduled_date', 'ASC']]
        });
...
router.put('/tasks/:task_id/complete', verifyToken, async (req, res) => {
    try {
        ...
        await CropActivity.create({
            ...
            activity_type: task.task_type || 'completed_task',
            activity_date: new Date(),
            notes: `Completed: ${task.task_name} - ${task.description}`
        });
...
router.post('/tasks', verifyToken, async (req, res) => {
    try {
        const { crop_id, farm_id, crop_name, task_name, description, scheduled_date } = req.body;
        if (!crop_id || !farm_id) {
            return res.status(400).json({ success: false, message: 'crop_id and farm_id required' });
        }
        const task = await Task.create({
            farm_id,
            crop_id,
            crop_name: crop_name || 'Other',
            task_name,
            description,
            scheduled_date,
            completed: false,
...
// ==================== CROP HEALTH ASSESSMENT (unchanged) ====================
router.post('/crop-health', verifyToken, async (req, res) => {
    try {
        const { crop_id, health_status, notes } = req.body;
        if (!crop_id) {
            return res.status(400).json({ success: false, message: 'crop_id required' });
        }
        await CropActivity.create({
            crop_id,
        ...
        // Update crop health score (you might want to update the Crop model instead of Farm)
        let healthScore = health_status === 'excellent' ? 90 : health_status === 'good' ? 75 : health_status === 'fair' ? 60 : health_status === 'poor' ? 40 : 25;
        await Crop.update({ health_score: healthScore }, { where: { id: crop_id } });
        
        res.json({ success: true });
    } catch (error) {
        console.error('Error recording crop health:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});
...
// ==================== WEATHER RECOMMENDATIONS (unchanged) ====================
router.get('/weather-recommendations', verifyToken, async (req, res) => {
    try {
        ...
        const rec = recommendations[weather_condition] || recommendations['sunny'];
